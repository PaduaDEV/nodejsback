...
class PessoaDao extends AbstractDao
{
  public async incluirPessoa(pessoaVo : PessoaVo)
  {
    ...
    {
      throw new IncluirError("pessoa", "Erro ao inserir no banco de dados", 404, error);
    }
  }
  ...
  public async alterarPessoa(pessoaVo : PessoaVo)
  {
    ...
    {
      if(error instanceof AlterarError) {throw error} else {throw new AlterarError("pessoa", "Erro ao alterar no banco de dados", 404, error);}
    }
  }
  ...
  public async pesquisarPessoa(pessoaVoFiltroPesquisa : PessoaVo) : Promise<any>
  {
    try
    {
      let recursos : any[] = this.gerarSQLConsultarListar(pessoaVoFiltroPesquisa);
    ...
    {
      throw new ConsultarError("pessoa", "Erro ao pesquisar no banco de dados", 404, error);
    }
  }
